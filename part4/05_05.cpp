//@compile clang++ part4/05_05.cpp -std=c++2b -Wall -Wextra -Wpedantic
#include <cassert>
#include <cmath>
#include <compare>
#include <istream>
...
int main()
{
    ...
    auto pre  = ++r;
    assert(pre == Rational<int>(5, 2));

    auto post = r++;
    assert(post == Rational<int>(5, 2));
    assert(r    == Rational<int>(7, 2));

    auto pred  = --r;
    assert(pred == Rational<int>(5, 2));

    auto postd = r--;
    assert(postd == Rational<int>(5, 2));
